...
<p>plogger is Oracle APEX, dynamic action plugin, when configured on page load event, captures user interaction events, errors thrown by browser and xml http request (xhr) made by page, in realtime and logs them in plogger table.<p>
<p>APEX developers can use this plugin to understand "how users are using the page and application" by analyzing captured events and data, fix page errors thrown by browser and tune performance by optimizing server side processing by analyzing xhr </p>
...
<p> Analyze events, error and xhr generated by your exploration, in plogger reports, which contains 
    <ul>
    <li>scatter chart to help you visualize events, errors and xhr generated by your exploration of customer tracker application</li>
